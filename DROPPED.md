...
| **ROI → S2-grid tiling** via s2geometry (`rsutils.s2_grid_utils.get_s2_grids_gdf`) | rsutils | **deferred** | it's the **inference tiling step** (ROI → per-grid inference datacubes, one grid = one task, with `scale_fact` overlap) **and** a large-ROI memory tool; not needed for v1 download→datacube→flatten. Recipe pinned in `ROADMAP.md` §4 (`get_s2_grids_gdf` grid_size_km=5→res 11 + `gpd.overlay` clip; deps `s2`+`s2cell`). NB: line "rsutils grab-bag … s2/s2cell helpers dropped" below refers to *other* misc helpers — this tiling fn is the authoritative deferred item. | **P4 `run_inference`** (port into `fsd/grid.py`); also very large ROIs |
