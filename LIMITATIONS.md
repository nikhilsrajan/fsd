...
| **Inference-on-blob (`run_inference(roi=…, runner="aml")`) is VALIDATED through Phase 2 on the real cluster (2026-07-28); Phase 3 is BLOCKED by TODO #57.** Phase 0 (env + adapter smoke, D3/D4), Phase 1 (a single-MGRS-tile ROI tiled into 9 grid cells → 9 COGs + STAC — exposed + fixed the grids.geojson GDAL-on-blob bug, `9422a1a`), and Phase 2 (D6/D7 resume + D13 guard) all GREEN. **Phase 3 (1167-cell fan-out) dies in `create_datacube.setup()` on the adlfs `InvalidBlockList` concurrent-write race → TODO #57.** `deploy` and the pre-built-cubes `run_inference` path stay local-only (unchanged, D14 scope). | TODO #57 lands, then re-run Phase 3 | spec 38; TODO #57 |
...
| **The storage seam's concurrent blob writes race under load** — `create_datacube.setup()`'s 16-thread fan-out of per-cell writes through the one shared adlfs client hits Azure's `InvalidBlockList` on `commit_block_list` and the write fails (no retry). Transient (a re-write succeeds), but there's no retry yet, so a large setup (measured: 1167 cells, worsened by a flaky link) can die mid-way. | any large concurrent-write setup (esp. `run_inference(roi=…)` over a big ROI) | TODO #57 (fix designed); blocks runbook 38 Phase 3 |
