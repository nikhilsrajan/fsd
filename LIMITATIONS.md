...
| **`create_training_data`/`flatten_training_data`'s `runner="aml"` flatten reduce (`runners.run_aml_flatten`, spec 39 D3) is implemented but not yet validated on the real cluster.** All unit tests are mocked at the AML-client boundary. `runbooks/39-training-data-on-aml.md` Phases 0-2 are written but not yet run. | someone runs Phases 0-2 and reports back | spec 39 |
| **The single-node flatten reduce has a memory ceiling** — `np.concatenate` allocates one new contiguous array and copies every input, so peak memory ≈ 2x the flattened total (all per-cube arrays + the result). Comfortably within one node's RAM for the demo scale (~900 small cubes, tens-to-low-hundreds of MB); untested at 10⁴–10⁵ cells, where a streaming/partial-reduce would be needed. | someone flattens 10⁴+ cells in one call, or Phase 1's measured `data.npy` bytes come back surprisingly large | spec 39 D3 §9; TODO #56 |
