...
| 47 | [driver-side-honesty.md](47-driver-side-honesty.md) | current | **draft, awaiting sign-off** | issues #66 (refuse a stale `input.csv` work list), #65 (progress on the four silent AML legs) and #64 (diff the catalog before dispatching an MPC download), plus Part D **amending spec 45 D4** (caller misuse must raise, not return `pass: False`); five §7 questions open |
