...
| **CDSE creds delivered via blob JSON (`--creds-url`) sit as plaintext at rest on blob**, unlike the Key Vault path — used only because the operator has no KV *write* role on the demo timeline (`ForbiddenByRbac`). Mitigated by writing to a `_secrets/` prefix and deleting the file immediately after the run (`runbooks/37-download-on-aml.md` Phases 0/3); switch back to Key Vault once a write role lands. | a KV write role becomes available, or the blob creds file outlives a single run | spec 37 D5 REVISED |
