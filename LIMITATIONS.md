...
| **Inference-on-blob (`run_inference(roi=…, runner="aml")`) is VALIDATED through Phase 2 on the real cluster (2026-07-28); Phase 3's blocker (TODO #57) is FIXED, re-run pending.** Phase 0 (env + adapter smoke, D3/D4), Phase 1 (a single-MGRS-tile ROI tiled into 9 grid cells → 9 COGs + STAC — exposed + fixed the grids.geojson GDAL-on-blob bug, `9422a1a`), and Phase 2 (D6/D7 resume + D13 guard) all GREEN. **Phase 3 (1167-cell fan-out) had died in `create_datacube.setup()` on the adlfs `InvalidBlockList` concurrent-write race; the seam retry (TODO #57) now lands the fix driver-side.** `deploy` and the pre-built-cubes `run_inference` path stay local-only (unchanged, D14 scope). | user re-runs Phase 3 and confirms `phase3_result.json` | spec 38; TODO #57 |
...
| **✅ FIXED (TODO #57) — the storage seam's concurrent blob writes now retry a transient adlfs race.** `create_datacube.setup()`'s 16-thread fan-out of per-cell writes through the one shared adlfs client used to hit Azure's `InvalidBlockList` on `commit_block_list` with no retry (measured: 1167 cells, worsened by a flaky link, died mid-way). `storage.fs._write_with_retry` (exponential backoff, backend-agnostic, matched on the Azure **storage error codes** — never on adlfs's catch-all `"Failed to upload block"` wrapper, which would retry permanent auth/RBAC failures too) now wraps `save_npy`/`write_parquet`/the new `write_bytes`/`write_text`; `create_datacube.py`'s per-cell `geometry.geojson` write goes through `fs.write_text`. **A permanent failure still surfaces on the first attempt** — the retry only ever costs 15.5 s of backoff on a genuinely transient race. | — | TODO #57 (closed); unblocks runbook 38 Phase 3 |
| **`run_inference(roi=…)` tiles the ROI's CONVEX HULL into grid cells**, so an ROI of scattered polygons (e.g. `AT_2018_TRAIN`'s 900 fields across ~4 MGRS tiles) expands to a large, mostly-empty cell set (**measured: 1167 cells** at `grid_size_km=5`) — a heavy, region-wide crop-map fan-out, not one-cell-per-field. Not a bug (it's how you get a contiguous map), but a cost/scope surprise. For fewer cells: a compact contiguous ROI, a larger `grid_size_km`, or `inference_datacubes=` mode over pre-built per-field cubes. | a roi-mode run costs more than expected | spec 21/38; PROGRESS 2026-07-28 |
